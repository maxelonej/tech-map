...
function ShowModal() {
    ...
    recipeButton.addEventListener('click', () => {
        ...
        header.forEach((component, index) => {
            ...
            keys.forEach((key, keyIndex) => {
                ...
                if (key === 'percent') {
                    ...
                    console.log(`Компонент ${index + 1}, Ключ '${key}':`, component[key], "=>", component.totalPercent); // Логируем исходное и преобразованное значение
                
                    cell.textContent = component.totalPercent;
                    totalPercent += component.totalPercent;
                    console.log(`Общий процент после добавления компонента ${index + 1}:`, totalPercent); // Логируем общий процент после добавления
                } else {
        ...
        console.log("Итоговая сумма после сокращения до 4 символов после запятой:", totalPercent);
...
const collectData = () => {
    ...
    const output = document.getElementById('outputControlCollapsibleTable'); // замените 'outputId' на реальный идентификатор вашего элемента
    ...
    phases.forEach(phase => {
        ...
        const phaseDataEntries = [
            ['adress_phase_id', phase.querySelector('.adress_phase_id')?.value.trim()],
            ['equipment_id', Array.from(document.querySelectorAll('.equipment_id option:checked')).map(option => option.value)],       
            ['delivery_method_id', phase.querySelector('.delivery_method_id')?.value.trim()],
            ...
            ['mixing_intensity_id', phase.querySelector('.mixing_intensity_id')?.value.trim()],
            ['type_of_equipment_id', phase.querySelector('.type_of_equipment_id')?.value.trim()],
            ...
            ['rotation_mixing', phase.querySelector('.rotation_mixing')?.value.trim()],
            ...
            ['tool_id', Array.from(document.querySelectorAll('.tool_id option:checked')).map(option => option.value)],
        ...
        const phaseData = Object.fromEntries(Object.entries(Object.fromEntries(phaseDataEntries)).filter(([key, value]) => value !== '' && value !== undefined));
        ...
        dataTech['info']['consistency'] = document.getElementById('consistency').value;
...
function calculateTotalTime(phaseBlock) {
    ...
    components.forEach(component => {
        const timePrepare = parseFloat(component.querySelector('.time_prepare').value) || 0;
        const timeLoad = parseFloat(component.querySelector('.time_load').value) || 0;
        const timeProcessing = parseFloat(component.querySelector('.time_processing').value) || 0;
...
function createPhaseDataBlock(phaseData = null, selectsData = null, componentData = null) {
    ...
    collapseImage.src = 'https://'+domain+'/icons/icons8-развернуть-96.png';
    collapseImage.alt = 'Expand';
    collapseImage.style.width = '30px';
    collapseImage.style.height = '30px';
    collapseImage.style.cursor = 'pointer'; // Добавляем курсор в виде руки для интерактивности
    ...
    phaseDataTitle.className = 'card-title';
    ...
    function toggleCollapse() {
        ...
        collapseImage.src = isCollapsed ? 'https://'+domain+'/icons/icons8-развернуть-96.png' : 'https://'+domain+'/icons/icons8-свернуть-96.png';
    ...
    let labels1 = ["Завоздушивание", "Интенсивность перемешивания", "Температура загрузки", "Рабочая температура", "Оснастка"];
    labels1.forEach(labelText => {
        let formGroup = document.createElement('div');
        formGroup.className = 'form-group d-flex justify-content-between w-100';
        let label = document.createElement('label');
        label.textContent = labelText;

        if (labelText === "Завоздушивание") {
            let select = document.createElement('select');
            select.className = 'form-control airing_id width-200 margin-right-200 check-input disabled-ptk m9-input';
            ...
            select.value = phaseData && phaseData.hasOwnProperty('airing_id') ? phaseData.airing_id : '';

        } else if (labelText === "Интенсивность перемешивания") {
            let select = document.createElement('select');
            select.className = 'form-control mixing_intensity_id width-200 margin-right-200 check-input disabled-ptk m9-input';
            const defaultOption = document.createElement('option');
            defaultOption.textContent = '';
            defaultOption.value = '';
            select.appendChild(defaultOption);
            Object.entries(selectsData.mixing_intensity).forEach(([key, optionText]) => {
                let option = document.createElement('option');
                option.textContent = optionText;
                option.value = key;
                select.appendChild(option);
            ...
            formGroup.appendChild(label);
            formGroup.appendChild(select);
            select.value = phaseData && phaseData.hasOwnProperty('mixing_intensity_id') ? phaseData.mixing_intensity_id : '';

        } else if (labelText === "Температура загрузки") {
            let input = document.createElement('input');
            input.className = 'form-control time_load width-200 margin-right-200 check-input m9-input';
            input.type = 'number';
            input.value = phaseData && phaseData.hasOwnProperty('time_load') ? phaseData.time_load : '';
            ...
            let input = document.createElement('input');
            input.className = 'form-control time_work width-200 margin-right-200 check-input m9-input';
            input.type = 'number';
            input.value = phaseData && phaseData.hasOwnProperty('time_work') ? phaseData.time_work : '';
            input.disabled = false;
            input.addEventListener('input', checkAllInputs);
            formGroup.appendChild(label);
            formGroup.appendChild(input);
            ...
            select.className = 'form-control equipment_id width-200 margin-right-200 m9-input';
            select.disabled = false;
            const defaultOption = document.createElement('option');
            defaultOption.textContent = '';
            defaultOption.value = '';
            select.appendChild(defaultOption);
            Object.entries(selectsData.equipment_phase).forEach(([key, optionText]) => {
                let option = document.createElement('option');
                option.textContent = optionText;
                option.value = key;
                select.appendChild(option);
            ...
            if (Number.isInteger(JSON.parse(phaseData.equipment_id))) {
                select.value = phaseData && phaseData.hasOwnProperty('equipment_id') ? phaseData.equipment_id : '';
            }
            else {
    ...
    let labels2 = ["Тип реактора", "Тип мешалки", "Инструменты", "pH фазы", "Общее время на фазу (мин)"];
    labels2.forEach(labelText => {
        ...
        if (labelText === "Тип реактора" && currentPage === 'PTK') {
            let select = document.createElement('select');
            select.className = 'form-control type_of_equipment_id width-200 margin-right-200 check-input m9-input';
            const defaultOption = document.createElement('option');
            defaultOption.textContent = '';
            defaultOption.value = '';
            select.appendChild(defaultOption);
            Object.entries(selectsData.type_of_equipment).forEach(([key, optionText]) => {
                let option = document.createElement('option');
                option.textContent = optionText;
                option.value = key;
                select.appendChild(option);
            ...
            select.value = phaseData && phaseData.hasOwnProperty('type_of_equipment_id') ? phaseData.type_of_equipment_id : '';

        } else if (labelText === "Тип мешалки") {
            let select = document.createElement('select');
            select.className = 'form-control mixer_type_id width-200 margin-right-200 check-input m9-input';
            const defaultOption = document.createElement('option');
            defaultOption.textContent = '';
            defaultOption.value = '';
            select.appendChild(defaultOption);
            Object.entries(selectsData.mixer_type_phase).forEach(([key, optionText]) => {
                let option = document.createElement('option');
                option.textContent = optionText;
                option.value = key;
                select.appendChild(option);
            ...
            formGroup.appendChild(label);
            formGroup.appendChild(select);
            select.value = phaseData && phaseData.hasOwnProperty('mixer_type_id') ? phaseData.mixer_type_id : '';

        } else if (labelText === "Инструменты" && currentPage === 'PTK') {
            let select = document.createElement('select');
            select.setAttribute('name', 'tools[]');
            select.setAttribute('multiple', 'multiple');
            select.className = 'form-control tool_id width-200 margin-right-200 check-input m9-input';
            const defaultOption = document.createElement('option');
            select.disabled = false;
            defaultOption.textContent = '';
            defaultOption.value = '';
            ...
            if (Number.isInteger(JSON.parse(phaseData.tool_id))) {
                select.value = phaseData && phaseData.hasOwnProperty('tool_id') ? phaseData.tool_id : '';
            }
            else {
            ...
            let input = document.createElement('input');
            input.className = 'form-control ph width-200 margin-right-200 check-input disabled-ptk m9-input';
            input.value = phaseData && phaseData.hasOwnProperty('ph') ? phaseData.ph : '';
            input.disabled = false;
            input.addEventListener('input', checkAllInputs);
            formGroup.appendChild(label);
            formGroup.appendChild(input);

        } else if (labelText === "Общее время на фазу (мин)", currentPage === 'PTK') {
            let input = document.createElement('input');
            input.type = 'number'; // Устанавливаем тип ввода как 'number'
            input.className = 'form-control time_general width-200 margin-right-200 check-input m9-input';
            input.addEventListener('input', checkAllInputs);
            input.value = phaseData && phaseData.hasOwnProperty('time_general') ? phaseData.time_general : '';
            formGroup.appendChild(label);
            formGroup.appendChild(input);
            input.disabled = true;
        }
    ...
    commentInput.value = phaseData && phaseData.hasOwnProperty('commentary') ? phaseData.commentary : '';
    ...
    collapseImage.addEventListener('click', function() {
        let isCollapsed = collapseContainer.classList.contains('show');
        collapseImage.innerHTML = isCollapsed ? '<img src="https://'+domain+'/icons/icons8-развернуть-96.png" alt="Expand" style="width:   30px; height:   30px;">' : '<img src="https://'+domain+'/icons/icons8-свернуть-96.png" alt="Collapse" style="width:   30px; height:   30px;">';
    });

    return phaseDataBlock;
}

function createComponentsTable(phaseData = null, selectsData = null, componentData = null, data) {
    ...
    componentsBlock.style.cssText = 'overflow-x: auto; max-width: 100vw;';
    ...
    flexContainer.className = 'my-2 align-items-center';
    ...
    table.style.cssText = 'overflow-x: auto; display: block; max-width: 100%; margin-top: 10px;';

    let headerRow = document.createElement('tr');
    let headers = ["<>", "№", "Название компонента", "Объем в продукте, %", "Объем в фазе, %", "Вес в килограммах", "Функция компонента", "t режим", "pH (Вод-пок)", "Способ подачи", "Визуальный контроль", "Итоговое состояние фазы после ввода компонента"];

    if (currentPage === 'PTK') {
        headers.splice(7, 0, "Время подготовки", "Время загрузки", "Время переработки");
    }
    ...
    imgElement.src = 'https://'+domain+'/icons/icons8-add-64.png';
...
function updatePercentInPhasePer(phaseElement, event) {
    let componentPercentInputs = phaseElement.querySelectorAll('.percent');
    ...
    componentPercentInputs.forEach(input => {
        totalPercentInPhase += parseFloat(input.closest('tr').querySelector('.percentInPhase').value);
    });

    componentPercentInputs.forEach(input => {
        let componentPercentValue = parseFloat(input.value);
        let percentInPhase = (componentPercentValue / totalComponentPercentValue) * 100;
        let percentInPhaseElement = input.closest('tr').querySelector('.percentInPhase');
...
function updateblePercentInPhase(phaseElement, event) {
    let componentWeightInputs = phaseElement.querySelectorAll('.componentWeight');
...
function createCell(type, value, selectData, textContent = null, percentInPhase = null) {
    ...
    if (type === 'select' && selectData) {
        ...
        select.addEventListener('click', function() {
            ...
            navigator.clipboard.writeText(thirdCell.textContent).then(function() {
    ...
    // Заменяем создание элемента для componentWeight на percent
        ...
        let input = document.createElement('input');
        input.type = 'number';
        input.className = 'form-control check-input componentWeight disabled-ptk m9-input'; // Используем класс вместо id
        input.disabled = false;
        input.min = 0;
        input.max = 100;
        input.addEventListener('input', function (event) {
...
async function importTechMap(urlupdate, tableId, recipeVersion, id, currentPage) {
    try {
        let bodyObject;
...
function addNewRowToTable(table, compDataFull, componentGood, selectsData) {
    ...
    dragIcon.setAttribute('src', 'https://img.icons8.com/ios/100/drag-reorder.png');
    ...
    newComponentNumberCell.textContent = table.querySelectorAll('.component').length + 1;
    newRow.appendChild(newComponentNumberCell);

    // Создаем выпадающий список для выбора компонента
    let availableComponents = compDataFull.filter(comp => componentGood.includes(comp.ID));
    let nameSelectCell = createCell('select', null, availableComponents.reduce((acc, comp) => {
    ...
    newRow.appendChild(createCell('percent', firstAvailableComponent.percent || '0')); // Объем в продукте, %
    newRow.appendChild(createCell('percentInPhase', '')); // Объем в фазе, %
    newRow.appendChild(createCell('componentWeight', '')); // Вес / кг
    newRow.appendChild(createCell('select_block', firstAvailableComponent.function_component_id, selectsData.function_component));
    if (currentPage === 'PTK') {
        newRow.appendChild(createCell('time_prepare', firstAvailableComponent.time_prepare || '')); // Время подготовки
        newRow.appendChild(createCell('time_load', firstAvailableComponent.time_load || '')); // Время загрузки
        newRow.appendChild(createCell('time_processing', firstAvailableComponent.time_processing || '')); // Время переработки
    }
    newRow.appendChild(createCell('number', firstAvailableComponent.temp_mode || ''));
    newRow.appendChild(createCell('input', firstAvailableComponent.phNorma || ''));
    newRow.appendChild(createCell('select_block', firstAvailableComponent.delivery_method_id, selectsData.delivery_method));
    newRow.appendChild(createCell('select_block', firstAvailableComponent.visual_control_id, selectsData.visual_control));
    newRow.appendChild(createCell('select_block', firstAvailableComponent.final_state_id, selectsData.final_state));
    ...
    deleteImage.setAttribute('src', 'https://'+domain+'/icons/icons8-cross-mark-96.png');
...
function createTableFromData(table, componentData = null, selectsData, percent = 0) {
    // Сортируем компоненты по их порядку
    const sortedComponents = Object.values(componentData).sort((a, b) => a.number - b.number);
    ...
    sortedComponents.forEach((component, index) => {
        ...
        row.appendChild(createCell('select name', component.component_id, selectsData, component.name));
        row.appendChild(createCell('percent', component.percent || '0')); // Объем в продукте, %
        row.appendChild(createCell('percentInPhase', component.percentInPhase || '0')); // Объем в фазе, %
        row.appendChild(createCell('componentWeight', component.componentWeight || '0')); // Вес / кг
        row.appendChild(createCell('select_block', component.function_component_id, selectsData.function_component)); // Функция компонента
        if (currentPage === 'PTK') {
        row.appendChild(createCell('time_prepare', component.time_prepare || '0')); // Время подготовки
        row.appendChild(createCell('time_load', component.time_load || '0')); // Время загрузки
        row.appendChild(createCell('time_processing', component.time_processing || '0')); // Время переработки
        }
        ...
        row.appendChild(createCell('select_block', component.delivery_method_id, selectsData.delivery_method)); // Способ подачи
        row.appendChild(createCell('select_block', component.visual_control_id, selectsData.visual_control)); // Визуальный контроль
        row.appendChild(createCell('select_block', component.final_state_id, selectsData.final_state)); // Итоговое состояние фазы после ввода компонента
...
function createMixingModeBlock(phaseData = null, selectsData = null, componentData = null, dataversion = null) {
    ...
    phaseMixingTitle.className = 'card-title';
    ...
    let labels1 = ["Адресная фаза", "Интенсивность перемешивания", "t подачи", "Скорость вращения (обороты/сек)"];
    labels1.forEach(labelText => {
        let formGroup = document.createElement('div');
        formGroup.className = 'form-group d-flex justify-content-between w-100';
        let label = document.createElement('label');
        ...
        if (labelText === 'Адресная фаза') {
            ...
            select.className = 'form-control adress_phase_id width-200 margin-right-200 check-input disabled-ptk m9-input';
            const defaultOption = document.createElement('option');
            defaultOption.textContent = '';
            defaultOption.value = '';
            select.appendChild(defaultOption);
            ...
            formGroup.appendChild(select)
            select.value = phaseData && phaseData.hasOwnProperty('adress_phase_id') ? phaseData.adress_phase_id : '';

        } else if (labelText === 'Интенсивность перемешивания' && currentPage === 'PTK') { //feed_type_id
            let select = document.createElement('select');
            select.className = 'form-control feed_type_id width-200 margin-right-200 m9-input';
            const defaultOption = document.createElement('option');
            defaultOption.textContent = '';
            defaultOption.value = '';
            select.appendChild(defaultOption);
            ...
            formGroup.appendChild(label);
            formGroup.appendChild(select);
            select.value = phaseData && phaseData.hasOwnProperty('feed_type_id') ? phaseData.feed_type_id : '';

        } else if (labelText === "t подачи") {
            let input = document.createElement('input');
            input.className = 'form-control time_supply width-200 margin-right-200 check-input disabled-ptk m9-input';
            input.type = 'number';
            input.value = phaseData && phaseData.hasOwnProperty('time_supply') ? phaseData.time_supply : '';
            input.addEventListener('input', checkAllInputs);
            input.disabled = false;
            formGroup.appendChild(label);
            formGroup.appendChild(input);

        } else if (labelText === 'Скорость вращения (обороты/сек)' && currentPage === 'PTK') {
            let input = document.createElement('input');
            input.className = 'form-control rotation_mixing width-200 margin-right-200 check-input m9-input';
            input.type = 'number';
            input.value = phaseData && phaseData.hasOwnProperty('rotation_mixing') ? phaseData.rotation_mixing : '';
            input.addEventListener('input', checkAllInputs);
            input.disabled = false;
            formGroup.appendChild(label);
            formGroup.appendChild(input);
    ...
    labels2.forEach(labelText => {
        ...
        if (labelText === 'Внешний вид фазы') {
            let select = document.createElement('select');
            select.className = 'form-control appearance_id width-200 margin-right-200 check-input disabled-ptk m9-input';
            const defaultOption = document.createElement('option');
            defaultOption.textContent = '';
            defaultOption.value = '';
            select.appendChild(defaultOption);
            ...
            formGroup.appendChild(label);
            formGroup.appendChild(select);
            select.value = phaseData && phaseData.hasOwnProperty('appearance_id') ? phaseData.appearance_id : '';

        } else if (labelText === 'Время на смешение') {
            let input = document.createElement('input');
            input.className = 'form-control time_to_mix width-200 margin-right-200 check-input m9-input';
            input.type = 'number';
            input.value = phaseData && phaseData.hasOwnProperty('time_to_mix') ? phaseData.time_to_mix : '';
            input.disabled = false;
            input.addEventListener('input', checkAllInputs);
            formGroup.appendChild(label);
            formGroup.appendChild(input);
            ...
            let select = document.createElement('select');
            select.className = 'form-control delivery_method_id width-200 margin-right-200 check-input disabled-ptk m9-input';
            const defaultOption = document.createElement('option');
            defaultOption.textContent = '';
            defaultOption.value = '';
            select.appendChild(defaultOption);
            Object.entries(selectsData.delivery_method).forEach(([key, optionText]) => {
                let option = document.createElement('option');
                option.textContent = optionText;
                option.value = key;
                select.appendChild(option);
            ...
            formGroup.appendChild(label);
            formGroup.appendChild(select);
            select.value = phaseData && phaseData.hasOwnProperty('delivery_method_id') ? phaseData.delivery_method_id : '';
        }
...
function addTableRow(tbody, data, dataversion) {
    ...
    sortedHeaderComponents.forEach(component => {
        ...
        const inciSelect = createSelectElement(component.inci, data.data.components, 'inci', headerComponents, component.component_id);
        inciCell.appendChild(inciSelect);

        // Создаем ячейку с select для Chemical Name
        const chemicalNameCell = document.createElement('td');
        const chemicalNameSelect = createSelectElement(component.chemicalName, data.data.components, 'chemicalName', headerComponents, component.component_id);
        chemicalNameCell.appendChild(chemicalNameSelect);

        // Создаем ячейку с select для Provider
        const providerCell = document.createElement('td');
        const providerSelect = createSelectElement(component.provider, data.data.components, 'provider', headerComponents, component.component_id);
        providerCell.appendChild(providerSelect);

        // Создаем ячейку с select для Name
        const nameCell = document.createElement('td');
        const nameSelect = createSelectElement(component.name, data.data.components, 'name', headerComponents, component.component_id);
...
function createSelectElement(selectedValue, components, propertyName, headerComponents, id) {
...
function createEmptyComponentRow(tbody, data, dataversion) {
    const row = document.createElement('tr');
    ...
    const selects = [inciSelect, compositionSelect, providerSelect, invoiceNameSelect];
    ...
    const selectizeInstances = selects.map(select => $(select).selectize(selectizeOptions)[0].selectize);
    ...
    const inciOptions = Array.from(new Map(data.data.components
        .filter(component => component.inci && component.isDeleted === "0")
        .map(component => [component.inci, { value: component.ID, text: component.inci }]))
        .values());

    updateSelectOptionsAndAutoSelect($(inciSelect), inciOptions, $(compositionSelect));

    let selectedInciValue = null;
    let selectedChemicalName = null;
    ...
    $(inciSelect)[0].selectize.on('change', function () {
        ...
        const selectedComponent = data.data.components.find(component => component.ID === selectedInciValue);
        ...
        if (selectedComponent) {
            ...
            const filteredComponents = data.data.components.filter(component =>
                component.inci === selectedTextinci && component.isDeleted === "0");
            console.log("filteredComponents:", filteredComponents);

            const chemicalNameOptions = filteredComponents.map(component => ({ value: component.ID, text: component.chemicalName }));
            console.log("chemicalNameOptions:", chemicalNameOptions);

            updateSelectOptionsAndAutoSelect($(compositionSelect), chemicalNameOptions, $(providerSelect));
            updateSelectOptionsAndAutoSelect($(providerSelect), [], $(invoiceNameSelect)); // Очищаем опции providerSelect и invoiceNameSelect
    ...
    $(compositionSelect)[0].selectize.on('change', function () {
        ...
        // Находим компонент с ID, соответствующим selectedValue
        const selectedComponent = data.data.components.find(component => component.ID === selectedChemicalNameValue);
        ...
        if (selectedComponent) {
            ...
            const filteredComponents = data.data.components.filter(component =>
                component.inci === selectedTextinci && component.chemicalName === selectedTextchemicalName && component.isDeleted === "0");
            ...
            if (filteredComponents.length >  0) {
                ...
                updateSelectOptionsAndAutoSelect($(providerSelect), providerOptions, $(invoiceNameSelect));
                updateSelectOptionsAndAutoSelect($(invoiceNameSelect), [], null); // Очищаем опции invoiceNameSelect
            } else {
                console.log(`No matching components found for selectedTextinci: ${selectedTextinci} and selectedTextchemicalName: ${selectedTextchemicalName}.`);
            }
        } else {
    ...
    $(providerSelect)[0].selectize.on('change', function () {
        ...
        // Находим компонент с ID, соответствующим selectedValue
        const selectedComponent = data.data.components.find(component => component.ID === selectedProviderValue);
        ...
        if (selectedComponent) {
            ...
            const filteredComponents = data.data.components.filter(component =>
                component.inci === selectedTextinci && component.chemicalName === selectedTextchemicalName && component.provider === selectedTextprovider && component.isDeleted === "0");
            console.log("Filtered components:", filteredComponents);

            // Создаем массив опций для invoiceNameSelect, используя значения name из отфильтрованных компонентов
            const invoiceNameOptions = filteredComponents.map(component => ({ value: component.ID, text: component.name }));
            console.log("Invoice name select options:", invoiceNameOptions);

            updateSelectOptionsAndAutoSelect($(invoiceNameSelect), invoiceNameOptions, null);
        } else {
            console.log("Компонент с таким provider не найден:", selectedProviderValue);
        }
    });
...
function updateCollapsibleTableContainer(collapsibleContainer, data, dataversion) {
...
function updateOutputControlCollapsibleContainer(collapsibleOutputContainer, data, dataversion) {
    // Находим tbody внутри контейнера и очищаем его
    ...
    // Находим кнопку добавления в контейнере и обновляем ее обработчик событий
    const addButton = collapsibleOutputContainer.querySelector('.btn-circle-green');
    // Удаляем предыдущий обработчик события 'click', если он существует
    addButton.removeEventListener('click', addButton.onclick);
    // Добавляем новый обработчик события для добавления одной новой строки
    addButton.onclick = function () {
...
function createCollapsibleTableContainer(data = null, dataversion = null) {
    ...
    collapsibleContainer.className = 'm9-container';

    // Создаем заголовок карточки, который будет служить переключателем
    const collapsibleHeader = document.createElement('div');
    collapsibleHeader.className = 'm9-container';
    collapsibleHeader.style.cursor = 'pointer';
    collapsibleHeader.style.margin = '0px';
    collapsibleHeader.textContent = 'Компоненты';
    // Добавляем обработчик событий для заголовка
    collapsibleHeader.addEventListener('click', function() {
        // Получаем все дочерние элементы контейнера, кроме заголовка
        const children = Array.from(collapsibleContainer.children).slice(1);

        // Перебираем все дочерние элементы и скрываем их
    ...
    // Добавляем контейнер с таблицей в collapsibleContainer
    collapsibleContainer.appendChild(tableContainer);

    // Создаем заголовок таблицы
    const thead = document.createElement('thead');
    const headerRow = document.createElement('tr');
    const headers = ['INCI', 'Химическое название', 'Поставщик', 'Название по накладной'];
    headers.forEach(headerText => {
    ...
    addButtonComponent.classList.add('btn', 'btn-success', 'btn-circle-green', 'mt-2', 'block-ptk');
    ...
    collapsibleContainer.appendChild(addButtonComponent);

    // Возвращаем созданный контейнер
    return collapsibleContainer;
}

function createOutputControlCollapsibleContainer(data = null, dataversion = null) {
    // Создаем сворачиваемый контейнер с классами Bootstrap
    const collapsibleOutputContainer = document.createElement('div');
    collapsibleOutputContainer.className = 'm9-container';
    collapsibleOutputContainer.id = 'outputControlCollapsibleContainer';

    // Создаем заголовок карточки, который будет служить переключателем
    const collapsibleOutputHeader = document.createElement('div');
    collapsibleOutputHeader.className = 'm9-container collapsible';
    collapsibleOutputHeader.style.cursor = 'pointer';
    collapsibleOutputHeader.style.margin = '0px';
    collapsibleOutputHeader.textContent = 'Выходной контроль';
    collapsibleOutputHeader.addEventListener('click', function() {
        const children = Array.from(collapsibleOutputContainer.children).slice(1);
        children.forEach(function(child) {
            child.style.display = child.style.display === 'none'? '' : 'none';
        });
    });
    collapsibleOutputContainer.appendChild(collapsibleOutputHeader);
    ...
    collapsibleOutputContainer.appendChild(table);
    ...
    const parameters = ['Водородный показатель рН', 'Аромат', 'Консистенция', 'Цвет', 'Вязкость', 'Плотность', 'SPR (длина волны), нм'];
    const ids = ['ph', 'aroma', 'consistency', 'color', 'viscosity', 'density', 'spr'];
    ...
    return collapsibleOutputContainer;
}
...
// Раскрывающийся контейнер "Испытания"
function createCollapsibleContainerTests(data = null, dataversion = null) {
    // Создаем сворачиваемый контейнер с классами Bootstrap
    const createCollapsibleContainerTests = document.createElement('div');
    createCollapsibleContainerTests.className = 'm9-container';
    createCollapsibleContainerTests.id = 'CollapsibleContentTest';

    // Создаем заголовок карточки, который будет служить переключателем
    const collapsibleHeaderTests = document.createElement('div');
    collapsibleHeaderTests.className = 'm9-container';
    collapsibleHeaderTests.style.cursor = 'pointer';
    collapsibleHeaderTests.style.margin = '0px';
    collapsibleHeaderTests.textContent = 'Испытания';
    // Добавляем обработчик событий для заголовка
    collapsibleHeaderTests.addEventListener('click', function() {
        // Получаем все дочерние элементы контейнера, кроме заголовка
        const children = Array.from(createCollapsibleContainerTests.children).slice(1);
    ...
    createCollapsibleContainerTests.appendChild(collapsibleHeaderTests);
    ...
    createCollapsibleContainerTests.appendChild(collapsibleContentTests);
    ...
    table.style.marginTop = '20px';
    createCollapsibleContainerTests.appendChild(table);
    ...
    const headColumns = ['Показатели', 'Нормативы', 'На начало испытаний', 'Ускоренное старение', 'Температурное испытание', 'Нормальные условия'];
    ...
    rowLabels.forEach(([labelText, prefix]) => {
        ...
        const ids = ['standards', 'start_test_standards', 'accelerated_aging', 'temperature_test', 'normal_conditions'];
    ...
    const tempTestingInput = createFormInput('t на начало испытаний', 'temp_testing', dataversion.test.temp_testing);
    const recipeVersionInput = createFormInput('Версия рецептуры', 'recipe_version', dataversion.version);
    const testStartEndInput = createFormInput('Дата испытания начало\\конец', 'test_start_end', dataversion.test.test_start_end);

    formsRow.appendChild(tempTestingInput);
    formsRow.appendChild(recipeVersionInput);
    formsRow.appendChild(testStartEndInput);

    createCollapsibleContainerTests.appendChild(formsRow);
    ...
    createCollapsibleContainerTests.appendChild(notesTextarea);

    // Создаем контейнер для таблицы
    const tableContainer = document.createElement('div');
    tableContainer.classList.add('table-container');
    createCollapsibleContainerTests.appendChild(tableContainer);
    ...
    for (let i =   0; i <   3; i++) {
        ...
        for (let j =   0; j <   2; j++) {
            ...
            imageElement.src = 'https://'+domain+'/icons/icons8-add-image-96.png';
            ...
            deleteImage.src = 'https://'+domain+'/icons/icons8-cross-mark-96.png';
    ...
    function handleImageUpload(event) {
        ...
        for (const file of files) {
            ...
            reader.onload = function (e) {
                ...
                parentContainer.querySelector('.button-container').style.display = 'none';
    ...
    function handleImageDelete(target) {
        ...
        if (uploadedImage) {
            ...
            parentContainer.querySelector('.button-container').style.display = 'block';
    ...
    return createCollapsibleContainerTests;
}
...
function getPhaseNameList(dataversion) {
...
function disabledOption(selectElement) {
    ...
    allSelect.forEach(select => {
            ...
            selectOptions.forEach(option => {
                if (option.value === selectElement.value && option.parentElement !== selectElement) {
...
const createPhaseInterface = (phaseData = null, selectsData = null, dataversion = null, selectedPhases = [], data) => {
    // Внешняя переменная для отслеживания количества новых фаз
    dataversion.phases
    let newPhaseCounter = 1;
    phaseCounter++;
    const container = document.createElement('div');
    container.className = 'card phase m9-container';
    ...
    containerHeader.style.justifyContent = 'flex-start'; // Добавляем это свойство
    ...
    populatePhaseSelect(numberPhaseSelect, selectedPhases, phaseData ? phaseData.num_phase : '');
    ...
    collapseContent.className = 'collapse show';
    ...
    let phaseDataBlock = createPhaseDataBlock(phaseData, selectsData, componentData); // добавляем блок с данными фазы
    collapseContent.appendChild(phaseDataBlock);

    let componentsTable = createComponentsTable(phaseData, selectsData, componentData, data); // добавляем таблицу компонентов
    collapseContent.appendChild(componentsTable);

    if (phaseData.name !== "/F/A") {
        let mixingModeBlock = createMixingModeBlock(phaseData, selectsData, componentData, dataversion); // добавляем блок смешивания
...
function collectSelectedValuesFromTable() {
    ...
    rows.forEach(row => {
        ...
        const select = cell.querySelector('select');
        if (select && select.value) { // Проверяем, что select существует и имеет значение
            selectedValues.push(select.value);
        }
        
        // Проверяем наличие input
        const input = cell.querySelector('input');
        if (input && input.textContent.trim()) { // Проверяем, что input существует и его текстовое содержимое не пустое
...
function checkAllInputs() {
    let allInputs = document.querySelectorAll('.check-input'); // Используйте класс .check-input
    let allFilled = Array.from(allInputs).every(input => input.value);
...
logo.src = 'https://static.tildacdn.com/tild3036-6565-4337-b762-306430326230/2c671ea40d8a4ffb8829.png'; // Большой логотип
logo.alt = 'Логотип';
...
// Создаем контейнер для логотипа
const hederNameContainer = document.createElement('div');
hederNameContainer.className = 'logo-container'; // Добавляем класс для стилизации
hederNameContainer.id = 'hederNameContainer'

// Добавляем контейнер логотипа в основной контейнер навигационного меню
menuContainer.appendChild(hederNameContainer);
...
const suportButton = document.createElement('span');
suportButton.className = 'm9-navigation-menu-icon'; // Добавляем класс к иконке
suportButton.innerHTML = `
...
`;
suportButton.style.cursor = 'pointer'; // Делаем иконку кликабельной
suportButton.onclick = function() {

};
menuItemsContainer.appendChild(suportButton);

// Создаем кнопку для перехода на рецептуру
const openNewTab = document.createElement('span');
openNewTab.className = 'm9-navigation-menu-icon'; // Добавляем класс к иконке
...
`
openNewTab.style.cursor = 'pointer'; // Делаем иконку кликабельной
...
refreshPageIcon.className = 'm9-navigation-menu-icon'; // Добавляем класс к иконке
...
`;
refreshPageIcon.style.cursor = 'pointer'; // Делаем иконку кликабельной
...
openPageIcon.className = 'm9-navigation-menu-icon'; // Добавляем класс к иконке
...
`;
openPageIcon.style.cursor = 'pointer'; // Делаем иконку кликабельной
...
infoButton.className = 'm9-navigation-menu-icon'; // Добавляем класс к иконке
...
`;
infoButton.style.cursor = 'pointer'; // Делаем иконку кликабельной
...
deleteButton.className = 'm9-navigation-menu-icon'; // Добавляем класс к иконке
...
`;
deleteButton.style.cursor = 'pointer'; // Делаем иконку кликабельной
deleteButton.onclick = function() {
...
notApproveButton.className = 'm9-navigation-menu-icon'; // Добавляем класс к иконке
...
`;
notApproveButton.style.cursor = 'pointer'; // Делаем иконку кликабельной
...
approveButton.className = 'm9-navigation-menu-icon'; // Добавляем класс к иконке
...
`;
approveButton.style.cursor = 'pointer'; // Делаем иконку кликабельной
...
copyButton.className = 'm9-navigation-menu-icon'; // Добавляем класс к иконке
...
`;
copyButton.style.cursor = 'pointer'; // Делаем иконку кликабельной
...
importButton.className = 'm9-navigation-menu-icon'; // Добавляем класс к иконке
...
`;
importButton.style.cursor = 'pointer'; // Делаем иконку кликабельной
...
redactButton.className = 'm9-navigation-menu-icon'; // Добавляем класс к иконке
...
`;
redactButton.style.cursor = 'pointer'; // Делаем иконку кликабельной
...
saveButton.className = 'm9-navigation-menu-icon'; // Добавляем класс к иконке
...
`;
saveButton.style.cursor = 'pointer'; // Делаем иконку кликабельной
...
(async () => {
    ...
    .then(data => {
          console.log(data);
          document.getElementById('headerMenuContainer').appendChild(createNavigationMenu(data)); // Предполагая, что createNavigationMenu принимает `data` как аргумент
          ShowModal();
            ...
            assembleButton.addEventListener('click', async () => {
                ...
                if (window.location.href.includes('diary-tasks')) {
                    ...
                    let emptyInputs = Array.from(allInputs).filter(input => input.value.trim() === '');
                    ...
                    if (!allFilled) {
                        ...
                        title.textContent = 'Пожалуйста, заполните все необходимые поля. Не заполнены:';
                    ...
                    let fullWeightValue = parseFloat(document.getElementById('full_weight').value);
                    ...
                    const isConfirmed = confirm(`Вы точно хотите "Утвердить" данную версию ${currentPage}?`);
                    if (isConfirmed) {
                        saveTechMap();
                    ...
                    alert(`Утвердить версию ЛТК можно только через задачу на Утверждение лабораторной техкарты`);
                }
            });
            ...
            deletLTM.addEventListener('click', async () => {
                ...
                const isConfirmed = confirm('Вы точно хотите удалить данную версию ЛТК?');
                if (isConfirmed) {
                    let param = document.getElementById('versionSelect').value; // Получаем ID версии
            ...
            Object.entries(data.data["tech-map"]).forEach(([versionID, versionData]) => {
                let option = document.createElement('option');
                option.textContent = versionData?.laboratory_version ?? versionData.version; // Название версии
            ...
            document.getElementById('importButton').addEventListener('click', function() {
                if (currentPage === 'PTK') {
                    ...
                    if (tableId && id) {
                        importTechMap(`https://${domain}/9x/app/php/export-ptk-to-table.php`, tableId, null, id, currentPage);
                    }
                    ...
                    if (tableId && recipeVersion && id) {
                        importTechMap(`https://${domain}/9x/tech-map/importLtmHook.php`, tableId, recipeVersion, id, currentPage);
                    }
                }
            });
            ...
            select.addEventListener('change', function () {
                ...
                let collapsibleContainer = document.getElementById('collapsibleContent');
                if (!collapsibleContainer) {
                    // Если контейнер не существует, создаем его
                    collapsibleContainer = createCollapsibleTableContainer(data, dataversion);
                    phasesContainer.appendChild(collapsibleContainer);
                    // Если контейнер существует, обновляем его содержимое
                    updateCollapsibleTableContainer(collapsibleContainer, data, dataversion);
                }

                //------------Отрисовка контейнера "Выходной контроль"------------------------

                let outputControlContainer = document.getElementById('outputControlContainer');
                if (!outputControlContainer) {
                    const outputControlContainer = createOutputControlCollapsibleContainer(data, dataversion);
                    phasesContainer.appendChild(outputControlContainer);
                } else {
                    updateOutputControlCollapsibleContainer(outputControlContainer, data, dataversion)
                }

                if (currentPage === 'PTK') {
                    var container = document.getElementById('outputControlCollapsibleContent');
                    if (container) {
                ...
                if (!ContainerTest) {
                    const ContainerTest = createCollapsibleContainerTests(data, dataversion);
                ...
                if (phasesData.length > 0) {
                    ...
                    phasesData.forEach(phaseData => {
                        const phaseInterface = createPhaseInterface(phaseData, selectsData, dataversion, selectedPhases, data);
                        phasesContainer.appendChild(phaseInterface);
                    });
                }

                // Определяем контейнер для заголовка
                const headerContainer = document.getElementById('hederNameContainer'); // Предполагается, что у вас есть элемент с id="headerContainer" для заголовка
                ...
                if (isApproved) {
                    ...
                    phasesContainer.querySelectorAll('input, select, button').forEach(element => {
                    ...
                    document.querySelectorAll('.card .btn .btn-danger').forEach(button => {
                        button.style.display = 'block';
                    });
                    ...
                    if (currentPage === 'PTK') { 
                        ...
                        phasesContainer.querySelectorAll('.block-ptk').forEach(element => {
                            // const label = element.previousElementSibling;
                            // label.style.display = "none";
                            element.style.display = "none";
                        });
                        phasesContainer.querySelectorAll('.disabled-ptk').forEach(element => {
                    ...
                    // Если версия ЛТК утверждена, отображаем заголовок "Утвержденная ЛТК"
                        ...
                        const classesToRemove = ['.type_of_equipment', '.time_general', '.equipment_id', '.rotation_mixing', '.feed_type_id', '.type_of_equipment_id', '.tool_id'];
                        classesToRemove.forEach(className => {
                            phasesContainer.querySelectorAll(className).forEach(removeElementAndLabel);
                        });
                    }

                } else {
                    // Если версия не утверждена, отображаем кнопки и т.п
                    document.getElementById('assembleButton').style.display = 'inline-block';
                    document.getElementById('notApproveButton').style.display = 'inline-block';
                    if (currentPage === 'LTK') {
                    ...
                    if (currentPage === 'PTK') {
                        ...
                        phasesContainer.querySelectorAll('.airing_id, .mixing_intensity_id, .ph').forEach(element => {
                            element.disabled = true;
                        });
                        phasesContainer.querySelectorAll('.block-ptk').forEach(element => {
                            // const label = element.previousElementSibling;
                ...
                if (addPhaseButton && !isAddPhaseButtonHandlerAdded) {
                    addPhaseButton.addEventListener('click', function () {
                        const newPhaseInterface = createPhaseInterface(phasesData, selectsData, dataversion, selectedPhases, data);
